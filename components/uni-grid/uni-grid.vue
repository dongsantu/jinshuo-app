<template>
	<view :class="{'uni-grid-no-border':!showBorder,'uni-grid-no-out-border':showBorder && !showOutBorder}" class="uni-grid">
		<view v-for="(items,i) in gridGroup" :key="i" class="uni-grid__flex">
			<view v-for="(item,index) in items" :hover-start-time="20" :hover-stay-time="70" :key="index" :class="[index == columnNum ? 'uni-grid-item-last' : '','uni-grid-item-' + type]" :style="{visibility:item.seize ? 'hidden' : 'inherit'}" class="uni-grid-item" hover-class="uni-grid-item-hover" @click="onClick(i,index,item)">
				<view v-if="!item.seize" class="uni-grid-item__content" >
					<image :src="item.image" class="uni-grid-item-image" :class="{'small': size==='small'}" />
	...
	export default {
		...
		methods: {
			onClick(index, num,item) {
				...
				/* if(this.needAct){
					this.actId = item.categoryId
				} */
				if(item.url){
					let url = ''
					if(item.categoryId){
						url = item.url + '&categoryId='+ item.categoryId
					}else{
						url = item.url
					}
					uni.navigateTo({
